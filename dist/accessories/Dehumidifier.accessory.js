...
class DehumidifierAccessory extends Base_accessory_1.default {
    ...
    _registerCharacteristics(dps) {
        ...
        if (!this.device.context.noChildLock) {
            ...
            this._removeCharacteristic(service, Characteristic.LockTargetState);
        ...
        this.device.on('change', (changes, state) => {
            ...
            if (characteristicChildLock && changes.hasOwnProperty(this.getDp('ChildLock'))) {
                ...
                if (characteristicChildLock.value !== newChildLock)
                    characteristicChildLock.updateValue(newChildLock);
            }
            if (changes.hasOwnProperty(this.getDp('FanSpeed'))) {
                const newSpeed = this._getRotationSpeed(state);
                if (characteristicSpeed.value !== newSpeed)
                    characteristicSpeed.updateValue(newSpeed);
            }
        });
    }
    getActive(callback) {
...
//# sourceMappingURL=Dehumidifier.accessory.js.map