...
class RGBTWOutletAccessory extends Base_accessory_1.default {
    ...
    _verifyCachedPlatformAccessory() {
        ...
        if (outletService)
            this._checkServiceName(outletService, outletName);
        else
            outletService = this.accessory.addService(Service.Outlet, outletName, 'outlet');
        ...
        this.accessory.services.forEach((service) => {
            if ((service.UUID === Service.Outlet.UUID && service !== outletService) ||
    ...
    _registerCharacteristics(dps) {
        ...
        if (energyKeys.volts) {
            ...
            this._removeCharacteristic(outletService, EnergyCharacteristics.Volts);
        ...
        if (energyKeys.amps) {
            ...
            this._removeCharacteristic(outletService, EnergyCharacteristics.Amperes);
        ...
        if (energyKeys.watts) {
            ...
            this._removeCharacteristic(outletService, EnergyCharacteristics.Watts);
        ...
        this.device.on('change', (changes, state) => {
            ...
            switch (state[this.dpMode]) {
                case this.cmdWhite:
                    if (changes.hasOwnProperty(this.dpBrightness) &&
                        this.convertBrightnessFromHomeKitToTuya(characteristicBrightness.value) !==
                    ...
                    if (changes.hasOwnProperty(this.dpColorTemperature) &&
                        this.convertColorTemperatureFromHomeKitToTuya(characteristicColorTemperature.value) !==
            ...
            if (changes.hasOwnProperty(energyKeys.volts) && characteristicVolts) {
                ...
                if (characteristicVolts.value !== newVolts)
                    characteristicVolts.updateValue(newVolts);
            }
            if (changes.hasOwnProperty(energyKeys.amps) && characteristicAmps) {
                ...
                if (characteristicAmps.value !== newAmps)
                    characteristicAmps.updateValue(newAmps);
            }
            if (changes.hasOwnProperty(energyKeys.watts) && characteristicWatts) {
                ...
                if (characteristicWatts.value !== newWatts)
                    characteristicWatts.updateValue(newWatts);
            }
        });
    }
    ...
    _setHueSaturation(prop, callback) {
        ...
        const callEachBack = (err) => {
            async_1.default.eachSeries(callbacks, (cb, next) => {
                try {
                    cb(err || null);
                }
                catch (_ex) {
...
exports.default = RGBTWOutletAccessory;
//# sourceMappingURL=RGBTWOutlet.accessory.js.map