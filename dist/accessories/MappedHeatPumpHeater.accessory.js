 ...
 */
class MappedHeatPumpHeaterAccessory extends Base_accessory_1.default {
    ...
    _registerCharacteristics(dps) {
        ...
        this.roomToWaterMap.sort((a, b) => a.room - b.room);
        this.log.debug(`[MappedHeatPump] DP mapping - Active: ${this.dpActive}, Return: ${this.dpReturnTemperature}, WaterTarget: ${this.dpWaterTarget}`);
        ...
        this.log.debug(`[MappedHeatPump] Initial return temperature from DP ${this.dpReturnTemperature}: raw=${dps[this.dpReturnTemperature]}, converted=${this._getDividedState(dps[this.dpReturnTemperature], this.returnTemperatureDivisor)}°C`);
        ...
        let characteristicOutsideTemperature;
        if (this.dpOutsideTemperature && this.device.context.exposeOutsideSensor) {
            const services = this.accessory.services.filter((s) => s.UUID === Service.TemperatureSensor.UUID && s.displayName.includes('Outside'));
            if (services.length > 0) {
                ...
                this.log.debug(`[MappedHeatPump] Initial outside temperature from DP ${this.dpOutsideTemperature}: raw=${dps[this.dpOutsideTemperature]}, converted=${this._getDividedState(dps[this.dpOutsideTemperature], this.outsideTemperatureDivisor)}°C`);
            }
        }
        // Configure optional flow temperature sensor
        let characteristicFlowTemperature;
        if (this.dpFlowTemperature && this.device.context.exposeFlowSensor) {
            const services = this.accessory.services.filter((s) => s.UUID === Service.TemperatureSensor.UUID && s.displayName.includes('Flow'));
            if (services.length > 0) {
                ...
                this.log.debug(`[MappedHeatPump] Initial flow temperature from DP ${this.dpFlowTemperature}: raw=${dps[this.dpFlowTemperature]}, converted=${this._getDividedState(dps[this.dpFlowTemperature], this.flowTemperatureDivisor)}°C`);
            }
        }
        // Configure optional return temperature sensor
        let characteristicReturnSensor;
        if (this.dpReturnTemperature && this.device.context.exposeReturnSensor) {
            const services = this.accessory.services.filter((s) => s.UUID === Service.TemperatureSensor.UUID && s.displayName.includes('Return'));
            if (services.length > 0) {
        ...
        this.device.on('change', (changes, state) => {
            ...
            if (changes.hasOwnProperty(this.dpReturnTemperature)) {
                ...
                if (characteristicCurrentTemperature.value !== convertedTemp) {
                    this.log.debug(`[MappedHeatPump] Return temperature changed - DP ${this.dpReturnTemperature}: raw=${changes[this.dpReturnTemperature]}, converted=${convertedTemp}°C`);
            ...
            if (this.dpOutsideTemperature &&
                changes.hasOwnProperty(this.dpOutsideTemperature) &&
                characteristicOutsideTemperature) {
                const convertedOutside = this._getDividedState(changes[this.dpOutsideTemperature], this.outsideTemperatureDivisor);
                if (characteristicOutsideTemperature.value !== convertedOutside) {
                    this.log.debug(`[MappedHeatPump] Outside temperature changed - DP ${this.dpOutsideTemperature}: raw=${changes[this.dpOutsideTemperature]}, converted=${convertedOutside}°C`);
            ...
            if (this.dpFlowTemperature && changes.hasOwnProperty(this.dpFlowTemperature) && characteristicFlowTemperature) {
                ...
                if (characteristicFlowTemperature.value !== convertedFlow) {
                    this.log.debug(`[MappedHeatPump] Flow temperature changed - DP ${this.dpFlowTemperature}: raw=${changes[this.dpFlowTemperature]}, converted=${convertedFlow}°C`);
    ...
    getVirtualRoomTarget(callback) {
        callback(null, this.virtualRoomTarget);
    }
    setVirtualRoomTarget(value, callback) {
        ...
        this.log.info(`[MappedHeatPump] Virtual room target: ${clampedRoom}°C → mapped water target: ${clampedWater}°C (raw: ${waterTargetRaw})`);
        this.setState(this.dpWaterTarget, waterTargetRaw, (err) => {
            if (err)
                return callback(err);
    ...
    _mapRoomToWater(roomTemp) {
        ...
        for (let i = 0; i < map.length - 1; i++) {
            ...
            if (roomTemp >= lower.room && roomTemp <= upper.room) {
                ...
                this.log.debug(`[MappedHeatPump] Interpolating: room ${roomTemp}°C between [${lower.room}→${lower.water}] and [${upper.room}→${upper.water}] = water ${waterTemp.toFixed(1)}°C`);
                return waterTemp;
            }
        }
...
exports.default = MappedHeatPumpHeaterAccessory;
//# sourceMappingURL=MappedHeatPumpHeater.accessory.js.map