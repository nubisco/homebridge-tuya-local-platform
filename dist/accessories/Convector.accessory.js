...
class ConvectorAccessory extends Base_accessory_1.default {
    ...
    _registerCharacteristics(dps) {
        ...
        if (this.device.context.cmdLow) {
            if (/^[a-z0-9]+$/i.test(this.device.context.cmdLow))
                this.cmdLow = ('' + this.device.context.cmdLow).trim();
            else
                throw new Error("The cmdLow doesn't appear to be valid: " + this.device.context.cmdLow);
        }
        this.cmdHigh = 'HIGH';
        if (this.device.context.cmdHigh) {
            if (/^[a-z0-9]+$/i.test(this.device.context.cmdHigh))
                this.cmdHigh = ('' + this.device.context.cmdHigh).trim();
            else
                throw new Error("The cmdHigh doesn't appear to be valid: " + this.device.context.cmdHigh);
        }
...
exports.default = ConvectorAccessory;
//# sourceMappingURL=Convector.accessory.js.map