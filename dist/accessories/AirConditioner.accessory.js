...
const STATE_OTHER = 9;
class AirConditionerAccessory extends Base_accessory_1.default {
    ...
    constructor(...props) {
        ...
        if (this.device.context.cmdCool) {
            if (/^c[a-z]+$/i.test(this.device.context.cmdCool))
                ...
                throw new Error("The cmdCool doesn't appear to be valid: " + this.device.context.cmdCool);
        }
        this.cmdHeat = 'HEAT';
        if (this.device.context.cmdHeat) {
            if (/^h[a-z]+$/i.test(this.device.context.cmdHeat))
                ...
                throw new Error("The cmdHeat doesn't appear to be valid: " + this.device.context.cmdHeat);
        }
        this.cmdAuto = 'AUTO';
        if (this.device.context.cmdAuto) {
            if (/^a[a-z]+$/i.test(this.device.context.cmdAuto))
                ...
                throw new Error("The cmdAuto doesn't appear to be valid: " + this.device.context.cmdAuto);
        }
        ...
        if (!this.device.context.noRotationSpeed) {
            const fanSpeedSteps = this.device.context.fanSpeedSteps &&
    ...
    _registerCharacteristics(dps) {
        ...
        this.log.debug(`[AirConditioner] DP mapping - Active: ${this.dpActive}, CurrentTemp: ${this.dpCurrentTemperature}, Threshold: ${this.dpThreshold}, Mode: ${this.dpMode}, TempDivisor: ${this.temperatureDivisor}`);
        ...
        this.log.debug(`[AirConditioner] Initial current temperature from DP ${this.dpCurrentTemperature}: raw=${dps[this.dpCurrentTemperature]}, converted=${this._getDividedState(dps[this.dpCurrentTemperature], this.temperatureDivisor)}°C`);
        ...
        if (!this.device.context.noCool) {
            ...
            this._removeCharacteristic(service, Characteristic.CoolingThresholdTemperature);
        ...
        if (!this.device.context.noRotationSpeed) {
            ...
            this._removeCharacteristic(service, Characteristic.RotationSpeed);
        ...
        this.device.on('change', (changes, state) => {
            ...
            if (changes.hasOwnProperty(this.dpCurrentTemperature)) {
                ...
                if (characteristicCurrentTemperature.value !== convertedTemp) {
                    this.log.debug(`[AirConditioner] Temperature changed - DP ${this.dpCurrentTemperature}: raw=${changes[this.dpCurrentTemperature]}, converted=${convertedTemp}°C`);
...
//# sourceMappingURL=AirConditioner.accessory.js.map